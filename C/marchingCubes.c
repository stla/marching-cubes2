...
unsigned* Faces7(int* faces, size_t* p1, double* values, size_t l, unsigned j) {
  ...
  for(size_t i = 0; i < l; i++) {
    ...
    double b =
        C0 * (A1 - A0) + A0 * (C1 - C0) - D0 * (B1 - B0) - B0 * (D1 - D0);
...
unsigned** levCells(double*** A,
  ...
  for(unsigned k = 0; k < nz - 1; k++) {
    size_t** topTypes =
        faceType(toMatrix(A, nx, ny, k + 1), nx, ny, level, max);
    size_t** cellTypes = matricialSum(
        bottomTypes, scaleMatrix(16, topTypes, nx - 1, ny - 1), nx - 1, ny - 1);
    unsigned length;
    unsigned** goodcells01 =
        whichIndicesAndItems(cellTypes, nx - 1, ny - 1, &length);
...
unsigned* get_tcase(unsigned* types, size_t nrow) {
  unsigned CRF[256] = {
      1,  2,  2,  3,  2,  4,  3,  6,  2,  3,  4,  6,  3,  6,  6,  9,  2,  3,
      4,  6,  5,  7,  7,  12, 4,  6,  8,  10, 7,  15, 13, 6,  2,  4,  3,  6,
      4,  8,  6,  10, 5,  7,  7,  15, 7,  13, 12, 6,  3,  6,  6,  9,  7,  13,
      15, 6,  7,  12, 13, 6,  11, 7,  7,  3,  2,  5,  4,  7,  3,  7,  6,  15,
      4,  7,  8,  13, 6,  12, 10, 6,  4,  7,  8,  13, 7,  11, 13, 7,  8,  13,
      14, 8,  13, 7,  8,  4,  3,  7,  6,  12, 6,  13, 9,  6,  7,  11, 13, 7,
      15, 7,  6,  3,  6,  15, 10, 6,  12, 7,  6,  3,  13, 7,  8,  4,  7,  5,
      4,  2,  2,  4,  5,  7,  4,  8,  7,  13, 3,  6,  7,  12, 6,  10, 15, 6,
      3,  6,  7,  15, 7,  13, 11, 7,  6,  9,  13, 6,  12, 6,  7,  3,  4,  8,
      7,  13, 8,  14, 13, 8,  7,  13, 11, 7,  13, 8,  7,  4,  6,  10, 12, 6,
      13, 8,  7,  4,  15, 6,  7,  3,  7,  4,  5,  2,  3,  7,  7,  11, 6,  13,
      12, 7,  6,  15, 13, 7,  9,  6,  6,  3,  6,  12, 13, 7,  15, 7,  7,  5,
      10, 6,  8,  4,  6,  3,  4,  2,  6,  13, 15, 7,  10, 8,  6,  4,  12, 7,
      7,  5,  6,  4,  3,  2,  9,  6,  6,  3,  6,  4,  3,  2,  6,  3,  4,  2,
      3,  2,  2,  1};
...
unsigned* getR(unsigned* tcase, size_t nrow, size_t* nR) {
  ...
  for(size_t i = 0; i < nrow; i++) {
    unsigned tc = tcase[i];
    if(tc == 1 || tc == 2 || tc == 5 || tc == 8 || tc == 9 || tc == 11 ||
       tc == 14) {
...
double* average(double** lambdamu, double* w1, double* w2, size_t n) {
  double* lambda = lambdamu[0];
  double* mu = lambdamu[1];
...
double** GetPoints(size_t** cubeco,
                   double* values,
                   size_t* p1,
                   unsigned* x1,
                   unsigned* x2,
...
double** computeContour3d(double* voxelAsVector,
                          unsigned nx,
                          unsigned ny,
                          unsigned nz,
                          double max,
    ...
    *ntriangles = totalLength;
    ...
    for(short c = 0; c < 7; c++) {
      ...
      if(nR3 > 0) {  // crash si nR3=2
        ...
        free(faces3);
        unsigned* unlisted_edges3 =
            unlist_u(Edges2, Edges2Sizes, cases3, nR3, &outlength3);
        free(cases3);
        // printf("outlength3 bis: %u\n", outlength3); // outlength3 = nedge *
        // nR3 printf("nedge: %u\n", nedge);
        ...
        for(unsigned j = 0; j < ind3Size; j++) {
          ...
          if(lwrows > 0) {
            ...
            unsigned** ed =
                subsetMatrix(edgesp1index, wrows, wcols, lwrows, lwcols);
            free(wrows);
            free(wcols);
            ...
            free(edge1);
            double** points3 = GetPoints(cubeco3, values3, col0edrep, xx1, xx2,
                                         (size_t)totalLength3);
            ...
            free(points3);
            triangles = realloc(
                triangles, (*ntriangles + totalLength3) * sizeof(*triangles));
            if(triangles == NULL) {
